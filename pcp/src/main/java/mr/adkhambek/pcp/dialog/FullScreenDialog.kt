...
internal class FullScreenDialog(
    ...
    init {
        val contentView: View = View.inflate(getContext(), R.layout.dialog_full_screen, null)
        ...
        var listCountries: List<Country> = arrayListOf()

        editText
            .editFlow()
            .onStart {
                listCountries = fetchModel().data.countries.country
                emit("")
            }.map { name ->
                if (name.isEmpty()) listCountries
                else listCountries.filter { country ->
